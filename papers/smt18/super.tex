...
The E-unification problem $E \vdash s\approx t$ can be solved by reaching
...
\UIC{$\clauseWithSubst{\emptyset}{\Sigma \circ \sigma}$}\DP\\[12pt]

% SN
\AXC{$\clauseWithSubst{s \approx t}{\Sigma}$}
\AXC{$\clauseWithSubst{u \not\approx v}{\Sigma'}$}
\LL{SN}
\BIC{$\clauseWithSubst{\sigma''(u[p \leftarrow t] \not\approx v)}{\sigma'''}$}\DP
...
\LL{SP}
\BIC{$\clauseWithSubst{\sigma''(u[p \leftarrow t] \approx v)}{\sigma'''}$}\DP
$\text{if} \left\{ \begin{array}{l@{\quad}l}
\sigma'' = \text{mgu}(u_{|p}, s) & u_{|p} \not\in V\\
\sigma''(s) \not\preceq \sigma''(t) & \sigma''(u) \not\preceq \sigma''(v)\\
\multicolumn{2}{l}{
